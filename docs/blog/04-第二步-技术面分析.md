...
技术面负责从 K 线数据里算出「均线、MACD、KDJ、RSI、布林带、OBV、背离、量能」以及「入场/离场参考」，并把结果整理成一段「数值摘要」给后面的 LLM 用。本篇用通俗方式说明：这些指标是什么、怎么算、代码在哪。

> 更详细的技术指标说明与智能体提示词，见 [docs/技术指标说明与智能体提示词.md](../技术指标说明与智能体提示词.md)。
...
- **输出**：一个字典，包含均线趋势、MACD 摘要、KDJ 摘要、RSI、布林带、OBV、背离、量能、以及「入场参考 / 离场参考」的简短文字。
...
- 用涨跌幅的 EMA 算 RS，再得到 RSI(14)。>70 超买、<30 超卖。
- 结果放在 `rsi_summary`。

### 5. 布林带（Bollinger Bands）

- 中轨=SMA20，上下轨=中轨±2倍标准差。价格在上轨上方可能超买，下轨下方可能超卖。
- 结果放在 `bb_summary`，含 `bollinger_pct`（0～100，越近上轨越超买）。

### 6. OBV 能量潮

- 涨日加量、跌日减量，累加得到 OBV，再与 OBV 的 20 日均线比较。
- OBV 在均线上表示量价配合偏多，结果放在 `obv_summary`。

### 7. 背离检测

- **顶背离**：价格创新高，MACD 或 RSI 未创新高 → 可能见顶。
- **底背离**：价格创新低，MACD 或 RSI 未创新低 → 可能见底。
- 结果放在 `divergence_summary`（macd_top, macd_bottom, rsi_top, rsi_bottom）。

### 8. 量比

- 近期成交量 / 20 日均量。>1 放量，<1 缩量。放量突破近期高点更可信。
- 结果放在 `volume_context`。

### 9. 入场/离场参考（tech_levels）

- **离场**：跌破 MA20 约多少考虑减仓、跌破 MA60 考虑离场；再结合 ATR（约 1.5 倍 ATR 止损）给一个参考价位。
- **入场**：突破/站稳 MA20 约多少可考虑入场；若近期高点高于当前价，会给出「突破某价位为强势信号」；若量比≥1.5 会提示「放量突破更佳」。
...
### 10. 一句技术面摘要（tech_status_one_line）

- 系统会把上述指标综合成一句，如：「日线多头排列；MACD金叉；KDJ中性；RSI中性(45)；布林带60%；量比1.2；ATR%2.5%；顶背离」。
- LLM 可直接引用并略作展开。
...
- **MACD/KDJ/RSI/ATR 计算**：同文件里的 `_macd`、`_kdj`、`_rsi`、`_atr`
- **布林带、OBV**：`_bollinger`、`_obv`
- **背离检测**：`_detect_divergence`
- **入场/离场文案**：`_compute_entry_exit_levels(...)`
- **一句摘要**：`_build_tech_status_one_line(...)`
- **调用方**：`agents/full_analysis.run_full_analysis` 里先调 `get_technical_summary`，再把返回的 `trend_ma`、`macd_summary`、`kdj_summary`、`rsi_summary`、`bb_summary`、`obv_summary`、`divergence_summary`、`volume_context`、`tech_levels`、`tech_status_one_line` 等拼进 `_build_prompt` 的「技术面」段落。
