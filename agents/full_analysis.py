...
评分：<1-5的数字，仅数字>
交易动作：<仅一个词或短语：如 多头/空头/观望/观察/不交易/轻仓试探/禁止交易 等>
加仓价格：<数字或“—”；可结合上方【技术面入场/离场参考】中的均线/阻力位给出具体价位>
减仓价格：<数字或“—”；可结合上方离场参考中的跌破MA20/MA60或ATR止损位给出>
...
def _parse_llm_output(text: str) -> Dict[str, Any]:
    ...
    for line in (text or "").strip().split("\n"):
        ...
        if line.startswith("核心结论："):
            ...
            try:
                s = re.sub(r"[^\d.]", "", line.split("：")[-1].strip()) or "3"
                out["score"] = float(s) if s else 3
            except Exception:
                out["score"] = 3
        elif line.startswith("交易动作："):
            out["action"] = line.replace("交易动作：", "").strip() or "观望"
        elif line.startswith("加仓价格："):
            out["add_price"] = line.replace("加仓价格：", "").strip() or "—"
        elif line.startswith("减仓价格："):
            out["reduce_price"] = line.replace("减仓价格：", "").strip() or "—"
    return out
